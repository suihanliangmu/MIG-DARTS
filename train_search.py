...
###.\train_search.py --add_layers 6 --add_layers 12 --dropout_rate 0.1 --dropout_rate 0.4 --dropout_rate 0.7 --note note_of_this_run
...
parser.add_argument('--excluding_plus', action='store_true', default=False, help='use new excluding method')    ##########DDDDDDDDDDDDDDDBBBBBBBBBBBBBBBBBBB
parser.add_argument('--excluding_power', type=int, default=4, help='excluding power about the rank of prob')    ##########DDDDDDDDDDDDDDDBBBBBBBBBBBBBBBBBBB
parser.add_argument('--stable_rank', action='store_true', default=True, help='use stable_rank')                ##########DDDDDDDDDDDDDDDBBBBBBBBBBBBBBBBBBB
parser.add_argument('--stable_factor', type=int, default=0.9, help='early stopping')                            ##########DDDDDDDDDDDDDDDBBBBBBBBBBBBBBBBBBB
# parser.add_argument('--add_width', action='append', default=['0'], help='add channels')
parser.add_argument('--add_width', action='append', default=['0', '5', '10'], help='add channels')


# parser.add_argument('--note', type=str, default='try', help='note for this run')
parser.add_argument('--note', type=str, default='test_04', help='note for this run')
...
parser.add_argument('--add_layers', action='append', default=['0', '6', '9'], help='add layers')
...
def main():
    ...
    r_witches = []
    for i in range(14):
        n_switches.append([True for j in range(len(PRIMITIVES))])
        n_switches[i][1] = False

    for i in range(14):
        r_witches.append([True for j in range(len(PRIMITIVES))])
        r_witches[i][3] = False
    switches_normal = copy.deepcopy(n_switches)
    switches_reduce = copy.deepcopy(r_witches)
    print(switches_normal)
    print(switches_reduce)
    # To be moved to args
    num_to_keep = [4, 3, 1]
    num_to_drop = [3, 1, 2]
    ...
    for sp in range(len(num_to_keep)):
        ...
        for epoch in range(epochs):
            ...
            lr = scheduler.get_last_lr()[0]  ######get_lr() ----------- >  [0]get_last_lr()[0]
            ...
            ########################### DB_beginning
            DB_arch_param = model.module.arch_parameters()
            DB_normal_prob = F.softmax(DB_arch_param[0], dim=sm_dim).data.cpu().numpy()
            DB_reduce_prob = F.softmax(DB_arch_param[1], dim=-1).data.cpu().numpy()
            logging.info("DDDDDDDDDBBBBBBBBBBBBBBnormal_prob")
            logging.info(DB_normal_prob)
            logging.info("DDDDDDDDDBBBBBBBBBBBBBBreduce_prob")
            logging.info(DB_reduce_prob)
            ...
            if args.stable_rank:
                stability_nums = 0
                for i in range(14):
                    if is_stable(DB_normal_prob[i, :]):
                        stability_nums = stability_nums + 1

                stability_proportion = stability_nums / 14

                print(stability_proportion)
                if stability_proportion >= args.stable_factor:
                    break
        ...
        for i in range(14):
            ...
            if sp == len(num_to_keep) - 1:
                ...
                if args.excluding_plus:
                    drop = get_min_k_plus(normal_prob[i, :], num_to_drop[sp])
                else:
        ...
        for i in range(14):
            ...
            if sp == len(num_to_keep) - 1:
                ...
                if args.excluding_plus:
                    drop = get_min_k_plus(reduce_prob[i, :], num_to_drop[sp])
                else:
...
#############################DDDDDDDDDDDDBBBBBBBBBBBBBBBB

# Sort the probability values, The higher the probability value, the higher the rank, the lower the value
def get_rank(input_in):
    input = copy.deepcopy(input_in)
    tmp = np.argsort(input).tolist()
    res = []
    for i in range(len(tmp)):
        res.append(len(tmp) - tmp.index(i))

    return res

# Excluding strategy
def power_random_func(input, x):
    res = []
    power = []
    for num in input:
        power.append(math.pow(num, x))



    for num in power:
        res.append((num / sum(power)) * random.random())

    return res


def get_min_k_plus(input_in, k):
    input = copy.deepcopy(input_in)
    tmp = power_random_func(get_rank(input), args.excluding_power)
...
def is_stable(input_in):

    input = copy.deepcopy(input_in)
    average_prob = 1 / input.size
    if(input.max() > average_prob * 1.2):
        return True
    return False
